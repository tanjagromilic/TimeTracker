...
namespace TimeTrackerPMF.Controllers
{
    ...
    public class TimeEntriesController : Controller
    {
        ...
        {
            _logger.LogInformation(message: $"Getting a time entry with id: {id}");
            var timeEntry = await _dbContext.TimeEntries.Include(x=>x.Project).Include(x=>x.Project.Client).Include(x=>x.User).SingleOrDefaultAsync(x => x.Id == id);
        ...
        {
            _logger.LogInformation(message: $"Getting a page {page} of time entries with page size {size} ");

            var timeEntries = await _dbContext.TimeEntries.Include(x => x.Project).Include(x => x.Project.Client).Include(x => x.User).Skip((page - 1) * size).Take(size).ToListAsync();
